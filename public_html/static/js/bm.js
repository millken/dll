...
function finishkv(){
	...
	gd.innerHTML = "<form method='post' action='http://www.youcandown.com/?url="+escape(document.location)+"&bm=y' id='ycd'><input type='hidden' name='vid' value='"+video_id+"' /><input type='hidden' name='title' value='"+title+"' /><input type='hidden' name='fmt' value='"+fmt+"' /><input class='master-sprite-new yt-uix-button yt-uix-tooltip' type='submit' value='Download with KeepVid' /></form>";
	d.body.appendChild(gd);
	document.forms["ycd"].submit();
}
var data;
var d=document;
if(d.location.href.match(/youtube.com/i)){
	...
	document.location.href='http://www.youcandown.com/?url='+escape(window.location);
}